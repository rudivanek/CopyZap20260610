...
**Banner + focus:** The existing "Please fill in your Project Description and Product/Service Name." banner (`showIntentPolishBanner` in `CopyMakerTab.tsx`) is now also triggered from the wizard path: `CopyMakerTab`'s `onApplyToForm` callback calls `setShowIntentPolishBanner(true)` and focuses `projectDescriptionRef` when the incoming `templateData` includes a non-empty `fieldsWithPlaceholders` array, so the cursor lands in the first field the user needs to complete. The banner's JSX is now self-managing — it stays visible while either `projectDescription` or `productServiceName` is empty and disappears on its own once both are filled — and it is also dismissible via an × button. The previous 2-second auto-hide timer (which vanished before the user could act) is removed from the banner JSX; the legacy cross-route prefill effect still has its own 2s timer, which is specific to that path. Protective comments were added next to the banner state declaration and its JSX noting that both are used by the wizard hand-off path and must be kept when the old `CZ_PREFILL_TO_COPY_MAKER_V1` prefill effect is deleted in a planned cleanup.

**Files modified:** `src/components/wizard/PurposeRewriteMode.tsx`, `src/utils/inputHighlight.ts`, `src/components/copy-maker/CopyMakerTab/CopyMakerTab.tsx`.

**Wizard routing:** Purpose Rewrite has its own `purposeRewrite` mode value, third radio button, render branch, validation skip, and Back to Mode Selection footer action. The existing create and improve branches continue to render `WizardStep`; the improve branch keeps Analyze URL gated by `answers.mode === 'improve'` with all existing buttons and behavior. The Start Hub's Copy Wizard card now has a third Purpose Rewrite sub-option, while the separate standalone Purpose Rewrite card and `/quick-polish` route remain unchanged.
