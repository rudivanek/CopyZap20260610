...
Version: 1.0
Last Updated: 2026-06-29T14:00:00Z
...
- **Lists**: bullet and numbered list items mapped to real docx `bullet`/`numbering` paragraphs (no literal `•` characters). Numbered lists use an `AbstractNumbering` reference `eval-list` with `LevelFormat.DECIMAL`.
