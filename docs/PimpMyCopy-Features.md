...
**Follow-up — qualifier density, duplicate work, and decision-badge suppression (2026-08-08):** Three corrections to the above:

1. **Qualifier no longer repeats in compact rows.** The qualifier sentence rendered on every `SubScoreChips` instance, including the three `compact={true}` call sites — so a Spanish session with three versions showed six or seven copies of the same 90-character sentence across ranking rows, card headers, and expanded panels. The block condition is now `{!hasSignal && !compact && (` so the full sentence appears only in the expanded panel. The two muted chip `<span>`s carry `title={NO_SIGNAL_QUALIFIER}` so the explanation is one hover away in compact rows. The muted `Conversion —` chip already carries the meaning on its own.

2. **Each heuristic now runs once.** `calculateMultiScoreDisplay` previously called each heuristic twice — once through `estimateConversionDisplayScore`, once through `countConversionSignals` — running four full passes over the text where it used to run two. Each pass is dozens of `includes()` calls, and this runs per version per render (twice over in `RankingsSnapshotCard`, which calls it at both ~93 and ~191). It now calls the internal `computeConversion` / `computeTrust` helpers directly and reads `.score` and `.signals` off each, so each heuristic runs exactly once. Behaviour is identical — purely removing duplicate work.
...
1. **Digits go neutral — except the unknown band.** `getScoreColorClasses().text` (and therefore `getScoreTextClass`) now returns `text-gray-900 dark:text-gray-100` for the three scored bands (>= 80, >= 50, < 50). The `!score || score === 0` branch keeps `text-gray-500 dark:text-gray-400` — a muted grey, deliberately dimmer than a real score — so an unscored version's placeholder dash reads as "no value" rather than as a real zero. `font-bold` was removed from the returned string — weight is the caller's business, and several callers already set their own. This one change flips every real score number in the app to neutral ink, because all eight digit-colouring call sites go through it.
...
**Task 2 — Put a bar next to the number (two live sites):**

| File | Site | Change |
|---|---|---|
| `WinnerHeroCard.tsx` ~147 | `text-2xl font-black` session score | Added a `w-1 h-5` bar immediately before the number, coloured via `getScoreMarkClass`. The `Session` caption above provides the word, so bar + caption satisfies rule 3. |
| `VersionAnalysisCard.tsx` ~195 | `text-xl font-black` session score | Same bar treatment. |
...
The marks are square bars (`w-1 h-5`), not round dots. The app's Tailwind config sets `borderRadius.full` to `0px` deliberately — the design system removes rounded corners — so an inline `style={{ borderRadius: '9999px' }}` override would both break that system and hide itself from any future search for radius overrides. A square bar respects the zero-radius convention and needs no inline style. `getScoreMarkClass` was added to the imports in the two files that needed it (`WinnerHeroCard`, `VersionAnalysisCard`).

`ComparisonCard.tsx` was NOT given marks. Two mark additions were attempted there (an "Overall Score" inline mark and a per-detail-score mark) and then reverted. That component is never rendered (see the dead-code finding below), and the `overallScore` field it reads is never populated, so the change was decoration on dead code. Reverting it also returned the typecheck error count to 1326 on its own, since the third `overallScore` reference went with it.
...
**Dead-code findings for the batch (not this step):**
1. **`ComparisonCard.tsx` is confirmed-unrendered.** A repo-wide search for `<ComparisonCard` returns zero hits. It joins `MultiScoreDisplay.tsx` as dead code slated for the dead-code batch. The two mark additions attempted there were reverted for this reason.
2. **`enhancedExports.ts` line ~4704 prints `undefined/10` on a live export path.** Unlike `ComparisonCard.tsx`, the export IS rendered — so this is a live bug, not dead code. It needs its own fix in the dead-code batch (or sooner), separate from the 3a colour work.

**Verification:**
- `grep -rE "[a-z]+-rose-[0-9]" src/` returns zero results.
- `grep -rE "borderRadius: '9999px'" src/` returns zero results — no inline radius overrides remain.
- No real score number anywhere renders in green, amber or red — the winner hero score, a version analysis score, and the ScoreCard circle all use neutral ink. Each still communicates quality via a bar or a ring. An unscored version's placeholder dash renders in the dimmer `text-gray-500 dark:text-gray-400`, visibly distinct from a real zero.
- A failing score (< 50) and a passing score (>= 80) sit side by side and are distinguishable by the bar colour (red bar vs green bar) while the numbers themselves are the same neutral grey.
- Labels read `Excellent` / `Good` / `Needs work`, not shouty caps, everywhere `getScoreLabel` from `scoreColors.ts` is used.
- `npm run build` passes.
- `npx tsc --noEmit -p tsconfig.app.json` returns 1326 errors — back to the `996a761` baseline. The +1 introduced by the `ComparisonCard` `overallScore` reference is gone now that the mark addition was reverted. Zero new errors from this step.
- The marks are square bars, not round dots, consistent with the app's zero-radius design system.
...
## Purpose Rewrite as the Copy Wizard's Third Mode (2026-08-07)

**Feature:** The Copy Wizard now has three independent modes: Make new copy, Improve existing copy, and Purpose Rewrite. The existing create and improve modes remain unchanged, including the Improve existing copy label, its fields, and its Analyze URL section. Purpose Rewrite is an additional single-screen mode and does not replace or absorb either existing mode.

**Purpose Rewrite flow:** The new mode ports the Intent flow UI from the standalone Purpose Rewrite page while continuing to use the existing quick-polish service layer. Users paste copy, see a live word count, choose Plain text or HTML, select one of the eleven intent presets, and see only the audience, goal, tone, or CTA fields declared by that preset. Preset default tones are applied until the user explicitly chooses a tone. The mode also includes the existing special-instructions helper text, one/two/three variant selection, polishing, result copy actions, evidence-based micro-confirmation tags, recommendation highlighting, selection, and refinement. It intentionally omits save-to-output because the selected result is handed directly to Copy Maker.

**Copy Maker handoff:** Continuing with a selected variant calls the wizard's direct `onApplyToForm` prop and forces the Advanced form. The hand-off payload now sets `tab: 'improve'` so Copy Maker opens in Improve mode (not Make new copy), making the polished text visible in the Original Copy field; `originalCopy` receives the selected text; `section` receives the chosen intent preset's label (e.g. "Hero Headline + Subheadline", "Call to Action"), matching the old cross-route prefill mapping; the intent audience, goal, and CTA populate `targetAudience`, `keyMessage`, and `callToAction`; the source language is detected from the original input; `wordCount` is set to `Custom`; and `customWordCount` uses the original input's word count. The intent tone is converted through the shared `QUICK_POLISH_TONE_MAP`, with `Professional` as the fallback, so the Copy Maker Tone dropdown remains populated for neutral, premium, friendly, bold, and formal intents.

**Required-field flagging:** Purpose Rewrite has no equivalent input for Project Description or Product/Service Name, both of which are required. The hand-off now also passes `fieldsWithPlaceholders: ['projectDescription', 'productServiceName']`, and the form's highlight utility (`src/utils/inputHighlight.ts`) has been extended so that `getInputClassName` / `getTextareaClassName` highlight a field when EITHER its value contains a `[bracket]` placeholder (the legacy behaviour, unchanged) OR the field is listed in `fieldsWithPlaceholders` AND is currently empty. The empty check means each highlight self-clears the moment the user types into that field, with no extra bookkeeping. Both fields render highlighted-and-empty when Copy Maker opens, and un-highlight as the user fills each one. Fields that received data from the hand-off (Original Copy, Section, Target Audience, Key Message, Call to Action) are never highlighted.

**Flagged-empty highlight visibility (2026-08-07 fix):** The original flagged-empty treatment used only `bg-orange-50` (`#fff7ed`), an almost-white pale orange that was effectively invisible against the form's light background — so Product/Service Name appeared unhighlighted even though it was flagged, and Project Description only looked highlighted because of its auto-focus ring (`focus:ring-primary-500`, where `primary-500` is `#ff6b35`), not the flag. The two highlight triggers now render differently, deliberately: the legacy bracket-placeholder case keeps its background-tint-only treatment (unchanged, used by templates and prefills), while the flagged-empty required-field case now adds a visible ring — `bg-orange-50 dark:bg-orange-950/30 ring-2 ring-primary-500 dark:ring-primary-400` — so the highlight is obvious at a glance. A ring was chosen over a border because the base class string already contains `border border-gray-300`, and appending `border-primary-500` would create two competing border utilities whose winner depends on CSS output order rather than class order; ring utilities layer cleanly over border utilities with no conflict. The two cases are intentionally distinct: "this template left you a placeholder to replace" (low urgency, tint only) vs "this required field is empty and blocking you" (high urgency, tint + ring). Both `getInputClassName` and `getTextareaClassName` apply the same split. Comments in `inputHighlight.ts` note the distinction is deliberate.

**Banner + focus:** The existing "Please fill in your Project Description and Product/Service Name." banner (`showIntentPolishBanner` in `CopyMakerTab.tsx`) is triggered from the wizard path: `CopyMakerTab`'s `onApplyToForm` callback calls `setShowIntentPolishBanner(true)` and focuses `projectDescriptionRef` when the incoming `templateData` includes a non-empty `fieldsWithPlaceholders` array, so the cursor lands in the first field the user needs to complete. The banner's JSX is self-managing — it stays visible while either `projectDescription` or `productServiceName` is empty and disappears on its own once both are filled — and it is also dismissible via an × button. The old cross-route prefill effect, its timer, and its sessionStorage payload were removed with the standalone page.

**Standalone page removal (2026-08-07):** The former standalone Purpose Rewrite page and its `/quick-polish` route have been removed. Purpose Rewrite now has one entry point: the third mode inside Copy Wizard. The Purpose Rewrite links were removed from the main sidebar, both Copy Maker sidebars (including the legacy rollback sidebar), and the standalone Start Hub card; the Start with Copy Wizard card and its `purposeRewrite` sub-option remain. The old cross-route sessionStorage prefill mechanism and its unused payload type were removed, along with obsolete Dashboard reopen links and `quick-polish` session-scope handling. Existing sessions and saved outputs have no `quick-polish` records, so no redirect or compatibility route is retained. The `toneMapping.ts` shared mapping remains in place and is now used only by the wizard mode.

**Mobile trade-off:** The standalone Purpose Rewrite escape hatch was removed from the small-screen gate. Purpose Rewrite is now accessed through Copy Maker's wizard, and Copy Maker continues to require a full-size screen; Copy Snap remains available as the only small-screen escape hatch.

**Files modified:** `src/components/wizard/PurposeRewriteMode.tsx`, `src/utils/inputHighlight.ts`, `src/components/copy-maker/CopyMakerTab/CopyMakerTab.tsx`, `src/App.tsx`, `src/components/NavSidebar.tsx`, `src/components/copy-maker/CopyMakerSidebar.tsx`, `src/components/copy-maker/CopyMakerSidebarLegacy.tsx`, `src/components/StartHubModal.tsx`, `src/components/Dashboard.tsx`, `src/components/DesktopRequired.tsx`, `src/services/sessionService.ts`, `src/services/supabaseClient.ts`, `src/types/index.ts`, and `src/features/quickPolish/types.ts`.
