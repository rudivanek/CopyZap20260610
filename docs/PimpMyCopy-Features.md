...
- `src/prompts/copyzap-eval-prompt.md` — The evaluation prompt text. Instructs Claude to perform a two-step evaluation: (1) blind editorial/conversion scoring of every copy version across 6 dimensions; (2) a side-by-side comparison table of CopyZap app scores vs. Claude's expert scores; (3) per-version deep critique with concrete rewrite suggestions; (4) an overall verdict and top 3 actionable improvements. The prompt is imported as a raw string using Vite's `?raw` import syntax.
...
- Added `handleGenerateEvalReport`: calls `buildLLMEvaluationMarkdown` to get the eval file content, prepends the prompt, sends to `claude-sonnet-4-5` with `max_tokens: 16000`, stores result, shows preview, plays success sound.
