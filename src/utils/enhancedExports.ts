...
const EXPORT_I18N = {
  en: {
    original: 'ORIGINAL',
    generated: 'GENERATED',
    voiceAdaptation: 'VOICE ADAPTATION',
    geoPackage: 'GEO PACKAGE',
    blended: 'BLENDED',
    modified: 'MODIFIED',
    boosted: 'BOOSTED',
    winner: 'Winner',
    previewBanner: (pct: number) => `Preview Copy (${pct}%) — Generated by CopyZap`,
    previewNote: (pct: number) => `Preview only (${pct}%) — full version available on request.`,
    qualityScore: 'QUALITY SCORE',
    whyImproved: "WHY IT'S IMPROVED",
    ...
    wordCountVsTarget: 'Word Count vs Target',
    excellentMatch: 'Excellent match',
    goodMatch: 'Good match',
    acceptableMatch: 'Acceptable match',
    poorMatch: 'Poor match',
    optimizationSuggestions: 'OPTIMIZATION SUGGESTIONS',
    words: 'words',
    copyReportLabel: 'CopyZap \u2014 Copy Report',
    generatedLabel: 'Generated',
    variantEvaluated: 'variant evaluated',
    variantsEvaluated: 'variants evaluated',
    languageLabel: 'Language',
    generatedBy: 'Generated by CopyZap',
    backToTop: 'Back to top',
    keyStrengthsLabel: 'KEY STRENGTHS',
    suggestedImprovementsLabel: 'SUGGESTED IMPROVEMENTS',
    potentialPts: (n: number) => ` — Potential +${n} pts`,
  ...
  es: {
    original: 'ORIGINAL',
    generated: 'GENERADO',
    voiceAdaptation: 'ADAPTACIÓN DE VOZ',
    geoPackage: 'PAQUETE GEO',
    blended: 'MEZCLADO',
    modified: 'MODIFICADO',
    boosted: 'MEJORADO',
    winner: 'Ganador',
    previewBanner: (pct: number) => `Vista previa (${pct}%) — Generado por CopyZap`,
    previewNote: (pct: number) => `Solo vista previa (${pct}%) — versión completa disponible bajo solicitud.`,
    qualityScore: 'PUNTUACIÓN DE CALIDAD',
    whyImproved: 'POR QUÉ MEJORÓ',
    ...
    wordCountVsTarget: 'Palabras vs. objetivo',
    excellentMatch: 'Coincidencia excelente',
    goodMatch: 'Buena coincidencia',
    acceptableMatch: 'Coincidencia aceptable',
    poorMatch: 'Coincidencia baja',
    optimizationSuggestions: 'SUGERENCIAS DE OPTIMIZACIÓN',
    words: 'palabras',
    copyReportLabel: 'CopyZap \u2014 Reporte de Copy',
    generatedLabel: 'Generado',
    variantEvaluated: 'variante evaluada',
    variantsEvaluated: 'variantes evaluadas',
    languageLabel: 'Idioma',
    generatedBy: 'Generado por CopyZap',
    backToTop: 'Volver arriba',
    keyStrengthsLabel: 'PUNTOS FUERTES',
    suggestedImprovementsLabel: 'MEJORAS SUGERIDAS',
    potentialPts: (n: number) => ` — Potencial +${n} pts`,
    inputSummaryLabel: 'RESUMEN DE ENTRADA',
    ...
    brandValuesLabel: 'Valores de Marca',
...
const getStructuredWordCount = (structured: StructuredCopyOutput): number => {
  ...
  for (const section of structured.sections) {
    total += countWords(section.title) + countWords(section.content) + (section.listItems || []).reduce((sum, li) => sum + countWords(li), 0);
  }
  return total;
};
...
export const generateFullHtmlExportForCard = (
  ...
  // Variant type label
  let variantTypeLabel = t.generated;
  if (card.type === GeneratedContentItemType.Original) variantTypeLabel = t.original;
  else if (card.persona) variantTypeLabel = t.voiceAdaptation;
  else if (card.sourceDisplayName?.toLowerCase().includes('geo')) variantTypeLabel = t.geoPackage;
  else if (card.blendInstructions) variantTypeLabel = t.blended;
  else if (card.modificationInstruction) variantTypeLabel = t.modified;
  else if (card.boostIteration) variantTypeLabel = t.boosted;
  ...
  if (previewPercent) {
    const totalWords = isStructured && actualContent
      ? getStructuredWordCount(actualContent as StructuredCopyOutput)
      : countWords(typeof actualContent === 'string' ? actualContent : JSON.stringify(actualContent));
    previewMaxWords = Math.max(40, Math.round(totalWords * (previewPercent / 100)));
  }
    ...
    if (!analysis || hasError) {
      ...
      if (hasStrengths || hasImprovements) {
        ...
        if (hasStrengths) {
          ...
          html += `<span style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:0.07em;">${t.keyStrengthsLabel}</span>\n`;
          html += '</div>\n';
          html += '<ul style="margin:0;padding:0;list-style:none;">\n';
        ...
        if (hasImprovements) {
          ...
          html += `<span style="font-size:10px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:0.07em;">${t.suggestedImprovementsLabel}</span>\n`;
          html += '</div>\n';
          html += '<ul style="margin:0;padding:0;list-style:none;">\n';
...
`;
  ...
  ${t.generatedBy} &mdash; ${formatExportTimestamp()}
    ...
    const filename = previewPercent ? `${sanitizedDesc}_${dateTime}_preview${previewPercent}pct.html` : `${sanitizedDesc}_${dateTime}.html`;
