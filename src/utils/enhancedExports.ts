...
<body id="top">
${previewMaxWords ? '<div style="background:#111827;color:#ffffff;text-align:center;padding:8px;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;font-weight:600;">Preview Copy — Generated by CopyZap</div>\n' : ''}
`;
    ...
    if (contentCards.length > 0) {
      ...
      contentCards.forEach((card) => {
        htmlContent += generateFullHtmlExportForCard(card, targetWordCount, winnerVersionId, exportScoringContext, comparisonResult, previewMaxWords);
      });
    }
  ...
  Generated by CopyZap &mdash; ${formatExportTimestamp()}
    ...
    const filename = previewMaxWords ? `${sanitizedDesc}_${dateTime}_preview.html` : `${sanitizedDesc}_${dateTime}.html`;
