...
export const generateFullHtmlExportForCard = (
  ...
  if (previewPercent) {
    html += `<p style="margin-top:16px;font-size:12px;font-style:italic;color:#9ca3af;">Preview only (${previewPercent}%) — full version available on request.</p>\n`;
  }
  html += '</div>\n';
...
<body id="top">
${previewPercent ? `<div style="background:#111827;color:#ffffff;text-align:center;padding:8px;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;font-weight:600;">Preview Copy (${previewPercent}%) — Generated by CopyZap</div>\n` : ''}
`;
    ...
    if (contentCards.length > 0) {
      ...
      contentCards.forEach((card) => {
        htmlContent += generateFullHtmlExportForCard(card, targetWordCount, winnerVersionId, exportScoringContext, comparisonResult, previewPercent);
      });
    }
