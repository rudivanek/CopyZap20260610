...
function splitSections(text: string): { label: string; text: string }[] {
  const parseBlocks = (src: string): { label: string; text: string }[] => {
    ...
    for (const block of blocks) {
      ...
      let label = '';
      let body = trimmed;
      // A heading must have something under it. When the block is a SINGLE
      // short line, that line is content, not a label — most often it is the
      // headline, which `contentToPlainTextWithMarkers` emits as its own block.
      // Consuming it as a label left the section with an empty body, so
      // `headlineAndSub` returned '' and the report printed the placeholder
      // "Tu titular actual" in the client-facing blockquote (headlines of 4
      // words or fewer only — longer ones were never eligible to be labels).
