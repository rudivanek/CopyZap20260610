...
const MONO_STACK = '"Inter",system-ui,sans-serif';
...
body{margin:0;background:var(--paper);color:var(--ink);font-family:var(--sans);font-size:var(--fs-body);line-height:1.65;-webkit-font-smoothing:antialiased;padding-bottom:56px}
...
h1{font-size:clamp(34px,5.4vw,var(--fs-display));line-height:1.08;overflow-wrap:anywhere}
...
.cover{background:var(--paper);color:var(--ink);padding:56px 0 64px;border:0}
.brandbar{display:flex;justify-content:space-between;align-items:center;gap:20px;padding-bottom:34px;margin-bottom:44px;border-bottom:1px solid var(--line);flex-wrap:wrap}
.logo{font-family:var(--serif);font-size:21px;letter-spacing:-.02em;color:var(--ink)}
.logo span{color:var(--ink)}
.powered{font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted)}
.cover h1{color:var(--ink);max-width:20ch}
.cover .kicker{font-size:11px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:var(--ink);margin-bottom:20px}
.cover .stamp{font-size:var(--fs-small);color:var(--muted);margin-top:6px}
.journey{margin-top:48px;background:var(--white);border:1px solid var(--line);border-radius:4px;padding:34px 30px 30px}
.journey-head{font-size:11px;font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:var(--muted);margin-bottom:26px}
...
.stop .num{font-family:var(--serif);font-size:clamp(38px,7vw,60px);line-height:1;color:var(--line);font-variant-numeric:tabular-nums}
.stop .num small{font-size:.4em;color:var(--muted);margin-left:2px}
.stop.now .num{color:var(--ink)}
.stop.goal .num{color:var(--gain)}
.stop .lbl{font-size:12px;letter-spacing:.06em;color:var(--muted);margin-top:10px}
.stop .lbl b{display:block;color:var(--ink);font-weight:600;letter-spacing:0;font-size:13px}
.rail{height:2px;margin:26px 0 0;background:linear-gradient(90deg,var(--line) 0%,var(--gain) 100%);position:relative}
.rail i{position:absolute;top:-4px;width:10px;height:10px;border-radius:50%;background:var(--ink);border:2px solid var(--line);transform:translateX(-50%)}
.rail i.a{left:25%} .rail i.b{left:75%;background:var(--gain);border-color:var(--gain)}
.journey-foot{margin-top:24px;padding-top:20px;border-top:1px solid var(--line);font-size:14px;color:var(--muted);line-height:1.6}
.journey-foot b{color:var(--ink);font-weight:600}
...
.preview-box .md-h1,.v-body .sec .md-h1{font-family:var(--serif);font-size:22px;font-weight:600;color:var(--ink);margin:28px 0 12px;padding-bottom:10px;border-bottom:1px solid var(--line-soft)}
.preview-box .md-h1:first-child,.v-body .sec .md-h1:first-child{margin-top:0}
.preview-box .md-h2,.v-body .sec .md-h2{font-family:var(--sans);font-size:var(--fs-label);font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);margin:22px 0 8px}
.preview-box .md-h3,.v-body .sec .md-h3{font-family:var(--sans);font-size:13px;font-weight:700;color:var(--ink-soft);margin:18px 0 8px}
.preview-box .md-p,.v-body .sec .md-p{font-size:var(--fs-body);line-height:1.7;color:var(--ink-soft);margin:0 0 10px}
.preview-box .md-ul,.v-body .sec .md-ul{margin:0 0 10px;padding-left:22px;color:var(--ink-soft);font-size:15px;line-height:1.65}
.preview-box .md-li,.v-body .sec .md-li{margin-bottom:6px}
...
.v-scores .gain{font-size:14px;font-weight:600;color:var(--gain);text-align:right;font-variant-numeric:tabular-nums;white-space:nowrap}
.v-scores .gain small{display:inline;font-weight:400;color:var(--muted);font-size:12px;margin-left:6px;padding-left:6px;border-left:1px solid var(--line)}
...
.split .pos h5{color:var(--gain)} .split .neg h5{color:var(--bad)}
...
.split .neg li:before{content:"\\2192";color:var(--bad)}
...
.rank-row.head .pos,.rank-row.head .nm,.rank-row.head .tot,.rank-row.head .dl,.rank-row.head .cell{font-size:var(--fs-label);font-weight:700;color:var(--muted);letter-spacing:.11em}
...
.rank-row .cell{text-align:right;font-variant-numeric:tabular-nums;font-size:15px}
...
.road-total{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:18px 26px;border-top:1px solid var(--line);background:var(--paper)}
.road-total .lb{font-size:14px;color:var(--ink-soft)}
.road-total .vv{font-size:var(--fs-score-md);font-weight:700;color:var(--gain);font-variant-numeric:tabular-nums}
.road-total .vv small{font-size:.5em;color:var(--muted)}
.cta-mini{display:flex;justify-content:space-between;align-items:center;gap:20px;flex-wrap:wrap;background:var(--accent-soft);border:1px solid var(--line);border-radius:4px;padding:24px 26px;margin-top:30px}
.cover .site{display:inline-block;margin-top:14px;font-size:15px;color:var(--ink-soft);text-decoration:underline}
.btn{display:inline-block;padding:12px 22px;border-radius:4px;border:1px solid var(--ink);background:var(--white);color:var(--ink);font-size:14px;font-weight:600;text-decoration:none;white-space:nowrap}
...
.findings{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:26px}
.finding{background:var(--white);border:1px solid var(--line);border-radius:4px;padding:22px 24px}
.finding .tag{display:inline-block;font-size:10px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--bad);background:var(--bad-soft);padding:3px 8px;border-radius:2px;margin-bottom:10px}
.finding h4{font-size:16px;line-height:1.35;margin:0 0 8px}
.finding p{font-size:14.5px;line-height:1.6;color:var(--ink-soft);margin:0}
.v-body .sec.fade{position:relative;max-height:160px;overflow:hidden}
.v-body .sec.fade:after{content:"";position:absolute;left:0;right:0;bottom:0;height:90px;background:linear-gradient(rgba(255,255,255,0),var(--white))}
.paywall{display:flex;justify-content:space-between;align-items:center;gap:20px;flex-wrap:wrap;margin:8px 28px 24px;padding:20px 22px;background:var(--paper);border:1px solid var(--line);border-radius:4px}
.paywall .pw-t{flex:1;min-width:240px;font-size:14px;line-height:1.55;color:var(--ink-soft)}
.paywall .pw-t b{color:var(--ink)}
.cta-mini .t{flex:1;min-width:240px;font-size:15px;color:var(--ink-soft)}
.cta{background:var(--white);border:1px solid var(--line);border-radius:4px;padding:36px 34px}
.cta .acts{display:flex;gap:12px;flex-wrap:wrap;margin-top:22px}
.toc .delta{font-size:13px;font-weight:600;color:var(--gain);text-align:right;white-space:nowrap;font-variant-numeric:tabular-nums}
.toc .delta small{font-weight:400;color:var(--muted);font-size:12px;margin-left:6px;padding-left:6px;border-left:1px solid var(--line)}
.toc a.base .delta{color:var(--muted);font-weight:400}
.toc .name em{font-style:normal;font-weight:400;color:var(--muted)}
.rank.rank-4 .rank-row{grid-template-columns:34px 1fr 140px 90px}
.v-scores .gain.base{color:var(--muted);font-weight:400}
footer{background:var(--paper);color:var(--muted);padding:44px 0;font-size:13px;line-height:1.7;border-top:1px solid var(--line)}
footer strong{color:var(--ink);font-weight:600}
.disclaimer{margin-top:14px;font-size:12px;color:var(--muted);line-height:1.6}
.breadcrumb-nav {
  position: fixed;
  bottom: 0;
  left: 0;
...
@media(max-width:760px){
  ...
  .rank-row{grid-template-columns:28px 1fr 64px 64px;gap:10px}
  .rank-row .cell{display:none}
  .rank-row .dl{grid-column:2;text-align:left} .rank-row .tot{grid-column:4;grid-row:1}
  ...
  .rank.rank-4 .rank-row{grid-template-columns:28px 1fr 84px}
  .rank.rank-4 .rank-row .dl{grid-column:2;text-align:left}
  .rank.rank-4 .rank-row .tot{grid-column:3;grid-row:1}
}
`;
...
export function getGoogleFontLinkTag(_serifStack: string, _sansStack: string): string {
  const familyParams = `family=${GOOGLE_FONT_FAMILIES['Inter']}`;
