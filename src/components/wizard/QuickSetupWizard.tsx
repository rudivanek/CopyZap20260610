...
// Per-client "brief" fields that describe the site/business being analyzed. None of
// these are guaranteed to be re-populated by every AI suggestion call (jsonSuggestion
// can legitimately omit a field, e.g. it has no SEO-keyword generator at all), so a
// value left over from a *previous* client's session can silently survive the merge
// into formState and leak into that client's exported report — see
// buildClientReportData.ts, which reads these straight off formState with no
// per-analysis freshness check. `industryNiche` / `readerFunnelStage` /
// `readerSophistication` are handled separately below since they're only ever set for
// 'create' mode.
const STALE_CLIENT_FIELD_DEFAULTS: Record<string, any> = {
  keyMessage: '',
  desiredEmotion: '',
  callToAction: '',
  brandValues: '',
...
/**
 * Mutates `ensuredData` in place so that a fresh URL scrape never lets a previous
 * client's leftover input (keywords, brand values, key message, etc.) survive into
 * the new client's formState. Fields this analysis run actually populated (via
 * jsonSuggestion or explicit wizard answers) are left untouched; only fields that
 * would otherwise be silently omitted — and therefore fall back to whatever was in
 * formState before — are forced to an explicit blank default.
 */
function clearStaleClientFields(ensuredData: Record<string, any>, mode: string) {
  for (const [field, defaultValue] of Object.entries(STALE_CLIENT_FIELD_DEFAULTS)) {
    if (ensuredData[field] === undefined) {
      ensuredData[field] = defaultValue;
    }
  }
  // keywords/keywordsExplicit always reset regardless of jsonSuggestion output —
  // SEO keywords are never (re)generated by the suggestion call, so any non-blank
  // value here is guaranteed to be stale, not fresh.
  ensuredData.keywords = '';
  ensuredData.keywordsExplicit = false;
  // Only populated by jsonSuggestion/wizard answers for 'create' mode (see
  // overriddenData above) — for any other mode they'd otherwise carry over from
  // whatever the previous client's analysis left in formState.
  if (mode !== 'create') {
    if (ensuredData.industryNiche === undefined) ensuredData.industryNiche = '';
    if (ensuredData.readerFunnelStage === undefined) ensuredData.readerFunnelStage = '';
    if (ensuredData.readerSophistication === undefined) ensuredData.readerSophistication = '';
  }
}
...
const QuickSetupWizard: React.FC<QuickSetupWizardProps> = ({
  ...
  const handleApplyFromStep2 = async () => {
    ...
    try {
      ...
      // A URL was analyzed this wizard run — that means this data represents a NEW
      // site/client. Clear out any per-client fields left over from a previous
      // analysis in this session that this run didn't freshly populate, otherwise
      // they silently leak into this client's exported report.
      if (wizardState.answers.analyzedUrl) {
        clearStaleClientFields(ensuredData, wizardState.answers.mode);
      }
