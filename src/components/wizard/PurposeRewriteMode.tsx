...
const PurposeRewriteMode: React.FC<PurposeRewriteModeProps> = ({ onApplyToForm, onClose }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {results.length > 0 && <section className="space-y-4"><div className="flex items-center justify-between"><h3 className="text-lg font-semibold text-gray-900 dark:text-white">Choose your rewrite</h3><span className="text-xs text-gray-500">Recommended option is highlighted</span></div>{results.map((result, index) => <article key={`${index}-${result.text.slice(0, 12)}`} className={`rounded-2xl border p-5 ${selectedIndex === index ? 'border-primary-500 ring-2 ring-primary-500/20' : 'border-gray-200 dark:border-gray-700'} bg-white dark:bg-gray-900`}><div className="flex items-start justify-between gap-4"><div className="flex items-center gap-2"><span className="text-sm font-semibold text-gray-900 dark:text-white">Version {index + 1}</span>{index === recommendedIndex && results.length > 1 && <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-800">Recommended</span>}{result.isRefined && <span className="rounded-full bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-800">Refined</span>}</div><button type="button" onClick={() => setSelectedIndex(index)} className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${selectedIndex === index ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>{selectedIndex === index ? <><Check className="mr-1 inline h-3.5 w-3.5" /> Selected</> : 'Select'}</button></div><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-gray-700 dark:text-gray-200">{result.text}</p><div className="mt-4 flex flex-wrap gap-2">{buildMicroConfirmation({ intentId: result.intentId, tone: result.tone, contentType: result.contentType, isRefined: result.isRefined, sourceText: result.sourceText, outputText: result.text }).map((tag) => <span key={tag} className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-300">{tag}</span>)}</div><div className="mt-3 flex gap-2"><button type="button" onClick={() => void handleCopy(result.text, index)} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs text-gray-600 dark:border-gray-700 dark:text-gray-300">{copiedIndex === index ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />} Copy</button><button type="button" onClick={() => { setRefineIndex(index); setRefineNotes(''); }} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-3 py-1.5 text-xs text-gray-600 dark:border-gray-700 dark:text-gray-300"><RefreshCw className="h-3.5 w-3.5" /> Refine</button></div></article>)}<button type="button" disabled={selectedIndex === null} onClick={handleContinue} className="w-full rounded-xl bg-gray-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-gray-700 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-white dark:text-gray-900 dark:hover:bg-gray-200">Continue to Copy Maker</button></section>}
